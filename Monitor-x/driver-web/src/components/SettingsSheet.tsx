  ...
  return (
    ...
    >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-14 h-14 rounded-full flex items-center justify-center shrink-0" style={{ background: 'var(--purple-soft)' }}>
              <span className="font-bold text-[18px]" style={{ color: 'var(--purple)' }}>{initials(user?.name)}</span>
            </div>
            <div className="min-w-0">
              <div className="font-bold text-[16px] truncate">{user?.name}</div>
              {idLine && <div className="text-[12px]" style={{ color: 'var(--text-muted)' }}>{idLine}</div>}
            </div>
          </div>
        ...
        <div className="space-y-1">
          ...
          <button onClick={() => go('/vehicle-tracking')} className="w-full flex items-center gap-3 p-3 rounded-lg text-[15px] font-medium hover:bg-black/5">
            <MapPin size={20} style={{ color: 'var(--purple)' }} /> Vehicle Tracking
          </button>
          <button onClick={() => go('/about')} className="w-full flex items-center gap-3 p-3 rounded-lg text-[15px] font-medium hover:bg-black/5">
            <Info size={20} style={{ color: 'var(--purple)' }} /> About Us
          </button>
        </div>
